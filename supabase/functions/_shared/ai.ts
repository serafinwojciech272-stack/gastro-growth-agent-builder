...
export function modelsFor(task: AiTask): string[] {
  ...
  const configured = [primary, fallback].map((value) => value.trim()).filter(Boolean);
  const enabledRegistryIds = new Set(MODEL_REGISTRY.filter((model) => model.enabled).map((model) => model.id));
  const registered = configured.filter((model) => enabledRegistryIds.has(model));
  return [...new Set(registered.length ? registered : [FALLBACK].filter((model) => enabledRegistryIds.has(model)))].slice(0, MAX_ATTEMPTS);
}

export async function callOpenRouter(params: { task: AiTask; system: string; user: string; temperature?: number; selectedModel?: string }): Promise<AiCallResult> {
  ...
  for (let index = 0; index < models.length; index += 1) {
    ...
    try {
      ...
      if (!response.ok) { lastError = typeof payload?.error?.message === 'string' ? payload.error.message : `AI provider error (${response.status})`; continue; }
      const content = payload?.choices?.[0]?.message?.content;
      if (typeof content !== 'string' || !content.trim()) { lastError = 'AI returned an empty response'; continue; }
      const usage = payload?.usage && typeof payload.usage === 'object' ? { promptTokens: numberOrUndefined(payload.usage.prompt_tokens), completionTokens: numberOrUndefined(payload.usage.completion_tokens), totalTokens: numberOrUndefined(payload.usage.total_tokens) } : undefined;
      return { model, content: content.trim(), latencyMs: Date.now() - started, attempts: index + 1, usage };
    } catch (error) { lastError = error instanceof Error && error.name === 'AbortError' ? `AI request timed out after ${TIMEOUT_MS}ms` : error instanceof Error ? error.message : 'Unknown AI provider error'; }

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const response = await fetch(growthLoopUrl, { method: 'POST', headers: { Authorization: authorization, apikey: anonKey, 'Content-Type': 'application/json' }, body: JSON.stringify({ problem, source: 'website_builder', source_project_id: project.id, website_context: { project: { id: project.id, name: project.name, source_url: project.source_url, vertical: project.vertical, goal: project.goal, status: project.status }, qa: { releaseGate: qa.releaseGate ?? null, score: qa.score ?? null }, artifacts: { brand: brand ?? null, content: content ?? null, architecture: architecture ?? null, visual: visual ?? null } } }) });

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (problem.length < 8 || problem.length > 4000) return json({ error: 'Problem must contain 8-4000 characters.' }, 400);
    ...
    const ai = await callOpenRouter({
      ...
      temperature: 0.15,
      system: `You are GGA's autonomous growth strategist. Analyze the restaurant problem and produce one coherent growth plan. Never invent business metrics. Missing numeric baselines or targets must be null. Prefer measurable goals and low-risk actions. Return JSON only with diagnosis, root_causes, mission and actions. diagnosis max 700 chars; root_causes 2-5; actions 2-5. mission priority 0-100. Every action requires title, description, action_type, impact_score 0-100, effort_score 0-100, risk_level low|medium|high.`,
      user: JSON.stringify({ restaurant, problem }),
    });

    const plan = normalizePlan(parseJson<Partial<Plan>>(ai.content));
    const quality = evaluateStructuredOutput(plan, {
      required: ['diagnosis', 'root_causes', 'mission', 'actions'],
      arrays: ['root_causes', 'actions'],
      minItems: { root_causes: 2, actions: 2 },
      maxItems: { root_causes: 5, actions: 5 },
      maxStringLength: { diagnosis: 700 },
    });
    if (quality.score < 75) return json({ error: 'AI plan failed quality gate', quality_score: quality.score }, 422);
    ...
    const { data: analysis, error: analysisError } = await supabase.from('ai_analyses').insert({
      ...
      priority: priorityFromNumber(plan.mission.priority),
    }).select('id').single();
    if (analysisError) throw analysisError;

    const { data: mission, error: missionError } = await supabase.from('growth_missions').insert({
      restaurant_id: restaurant.id,
      ...
      metadata: { quality_score: quality.score, model: ai.model, problem },
    }).select('id,title,status,priority').single();
    if (missionError) throw missionError;

    const { data: actions, error: actionsError } = await supabase.from('growth_actions').insert(plan.actions.map((action) => ({
      mission_id: mission.id,
      restaurant_id: restaurant.id,
      ...action,
      status: 'proposed',
      created_by: user.id,
      metadata: { source: 'autonomous_growth_loop', quality_score: quality.score },
    }))).select('id,title,status,impact_score,effort_score,risk_level');
    if (actionsError) throw actionsError;

    const { error: eventError } = await supabase.from('growth_action_events').insert((actions ?? []).map((action) => ({
      action_id: action.id,
      restaurant_id: restaurant.id,
      event_type: 'proposed',
      metadata: { mission_id: mission.id, source_analysis_id: analysis.id },
      created_by: user.id,
    })));
    ...
    const { error: telemetryError } = await supabase.rpc('record_ai_run', {
      ...
      p_metadata: { pipeline: 'growth-loop', analysis_id: analysis.id, mission_id: mission.id },
    });
    if (telemetryError) console.error('Growth telemetry error:', telemetryError);

    return json({
      pipeline: 'observe-diagnose-plan-propose',
      analysis_id: analysis.id,
      mission,
      actions: actions ?? [],
      quality_score: quality.score,
      model: ai.model,
      next_step: 'customer_approval',
    });
  } catch (error) {
    console.error('GGA growth loop error:', error);
    return json({ error: error instanceof Error ? error.message : 'Unexpected growth loop error.' }, 502);
  }
});
...
function normalizePlan(input: Partial<Plan>): Plan {
  ...
  return {
    ...
    mission: {
      title: String(mission.title || 'Growth Mission').slice(0, 160),
      goal: String(mission.goal || 'Improve restaurant growth').slice(0, 500),
      priority: clamp(mission.priority, 0, 100, 50),
      target_value: numberOrNull(mission.target_value),
      baseline_value: numberOrNull(mission.baseline_value),
      unit: mission.unit == null ? null : String(mission.unit).slice(0, 40),
    },
    actions: actions.slice(0, 5).map((action) => ({
      ...
      impact_score: clamp(action?.impact_score, 0, 100, 50),
      effort_score: clamp(action?.effort_score, 0, 100, 50),

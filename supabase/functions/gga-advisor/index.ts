...
type AdvisorResult = { diagnosis: string; root_causes: string[]; recommendations: { title: string; rationale: string; priority: Priority; actions: string[] }[]; priority: Priority };
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (problem.length < 8) return json({ error: 'Describe the restaurant problem in at least 8 characters.' }, 400, corsHeaders);
    ...
    const ai = await callOpenRouter({ task: 'advisor', system, user: JSON.stringify({ restaurant, user_problem: problem }), temperature: 0.2, selectedModel: selected.model });
    ...
    const { error: telemetryError } = await supabase.rpc('record_ai_run', { p_restaurant_id: restaurant.id, p_task: 'advisor', p_model: ai.model, p_attempts: ai.attempts, p_latency_ms: ai.latencyMs, p_prompt_tokens: ai.usage?.promptTokens ?? null, p_completion_tokens: ai.usage?.completionTokens ?? null, p_total_tokens: ai.usage?.totalTokens ?? null, p_success: true, p_quality_score: quality.score, p_metadata: { checks: quality.checks, analysis_id: saved.id } });
    if (telemetryError) console.error('GGA telemetry error:', telemetryError);
    return json({ analysis_id: saved.id, created_at: saved.created_at, restaurant: { id: restaurant.id, name: restaurant.name }, model: ai.model, latency_ms: ai.latencyMs, attempts: ai.attempts, quality_score: quality.score, ...parsed }, 200, corsHeaders);
  } catch (error) {

...
export function buildOutcome(input: { missionId: string; actionId?: string; kpi: GrowthKpi; before?: number; after?: number; confidence: number; evidence?: string[] }): GrowthOutcome {
  ...
  return { missionId: input.missionId, actionId: input.actionId, status: classifyOutcome(input.confidence, metric.delta), measuredAt: new Date().toISOString(), metrics: { [input.kpi.key]: metric }, evidence: input.evidence ?? [], confidence: Math.max(0, Math.min(1, input.confidence)) };
}

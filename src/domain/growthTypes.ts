...
export type GrowthOutcome = {
  ...
  measuredAt: string;
  metrics: Record<string, { baseline?: number; before?: number; after?: number; delta?: number }>;
  evidence?: string[];
  confidence: number;
};

export type GrowthDecisionContext = {
  vertical: GrowthVerticalId;
  businessId: string;
  objective: string;
  kpis: GrowthKpi[];
...
export function isAutonomousAction(action: GrowthAction): boolean {
  return action.autonomyLevel >= 3 && !action.requiresApproval;
}

export function classifyOutcome(confidence: number, delta: number | undefined): OutcomeStatus {
  if (delta === undefined || confidence < 0.5) return "insufficient_data";
  if (delta > 0) return confidence >= 0.8 ? "success" : "partial_success";

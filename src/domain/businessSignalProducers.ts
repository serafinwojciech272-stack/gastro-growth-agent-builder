...
function id(prefix: string, businessId: string, observedAt: string): string {
  return `${prefix}:${businessId}:${observedAt}`;
}
  ...
  produce({ business, observedAt = new Date().toISOString() }, data) {
    ...
    for (const [metric, value] of scores) {
      ...
      const signal = createSignal({
        id: id("website", business.business.id, `${observedAt}:${metric}`), businessId: business.business.id,
      ...
      signals.push(signal);
      evidence.push(createEvidence({ id: `${signal.id}:evidence`, businessId: business.business.id, type: "observation", source: "website-intelligence", observation: `${metric} scored ${value}/100.`, data: { value, issues: data.issues ?? [] }, supportingSignalIds: [signal.id], confidence: 0.85, observedAt }));
    }
...
export const reviewSignalProducer: BusinessSignalProducer<ReviewIntelligenceInput> = {
  ...
  produce({ business, observedAt = new Date().toISOString() }, data) {
    ...
    if (rating !== undefined) {
      const signal = createSignal({ id: id("reviews", business.business.id, `${observedAt}:rating`), businessId: business.business.id, type: rating < 4 ? "threshold_breach" : "metric_change", source: "review-intelligence", metric: "reviews.rating", value: rating, direction: rating < 4 ? "negative" : "positive", confidence: confidence(data.reviewCount ? Math.min(1, data.reviewCount / 100) : undefined), context: { reviewCount: data.reviewCount ?? 0, negativeThemes: data.negativeThemes ?? [], positiveThemes: data.positiveThemes ?? [] }, observedAt });
      signals.push(signal);
      evidence.push(createEvidence({ id: `${signal.id}:evidence`, businessId: business.business.id, type: "review", source: "review-intelligence", observation: `Observed review rating ${rating.toFixed(2)}.`, data: { reviewCount: data.reviewCount ?? 0, negativeThemes: data.negativeThemes ?? [], positiveThemes: data.positiveThemes ?? [] }, supportingSignalIds: [signal.id], confidence: signal.confidence, observedAt }));
    }
...
export const menuSignalProducer: BusinessSignalProducer<MenuProductIntelligenceInput> = {
  ...
  produce({ business, observedAt = new Date().toISOString() }, data) {
    ...
    if (data.lowPerformers?.length) {
      const signal = createSignal({ id: id("menu", business.business.id, `${observedAt}:low-performers`), businessId: business.business.id, type: "opportunity", source: "menu-intelligence", metric: "menu.low_performers", value: data.lowPerformers.length, direction: "negative", confidence: 0.75, context: { items: data.lowPerformers }, observedAt });
      signals.push(signal);
      evidence.push(createEvidence({ id: `${signal.id}:evidence`, businessId: business.business.id, type: "observation", source: "menu-intelligence", observation: `${data.lowPerformers.length} low-performing menu items identified.`, data: { items: data.lowPerformers }, supportingSignalIds: [signal.id], confidence: 0.75, observedAt }));
    }
    if (data.categoryGaps?.length) {
      const signal = createSignal({ id: id("menu", business.business.id, `${observedAt}:category-gaps`), businessId: business.business.id, type: "opportunity", source: "menu-intelligence", metric: "menu.category_gaps", value: data.categoryGaps.length, direction: "negative", confidence: 0.7, context: { gaps: data.categoryGaps }, observedAt });
      signals.push(signal);
      evidence.push(createEvidence({ id: `${signal.id}:evidence`, businessId: business.business.id, type: "observation", source: "menu-intelligence", observation: `${data.categoryGaps.length} category gaps identified.`, data: { gaps: data.categoryGaps }, supportingSignalIds: [signal.id], confidence: 0.7, observedAt }));
    }
...
export const analyticsKpiSignalProducer: BusinessSignalProducer<AnalyticsKpiInput> = {
  ...
  produce({ business, observedAt = new Date().toISOString() }, data) {
    ...
    for (const [metric, value, baseline] of pairs) {
      ...
      const signal = createSignal({ id: id("analytics", business.business.id, `${observedAt}:${metric}`), businessId: business.business.id, type: Math.abs(deviation ?? 0) >= 10 ? "anomaly" : "metric_change", source: "analytics-kpi", metric: `kpi.${metric}`, value, baseline, deviation, direction, confidence: baseline === undefined ? 0.65 : 0.9, context: {}, observedAt });
      signals.push(signal);
      evidence.push(createEvidence({ id: `${signal.id}:evidence`, businessId: business.business.id, type: "measurement", source: "analytics-kpi", observation: baseline === undefined ? `${metric} measured at ${value}.` : `${metric} changed ${deviation!.toFixed(1)}% versus baseline.`, data: { value, baseline, deviation }, supportingSignalIds: [signal.id], confidence: signal.confidence, observedAt }));
    }
...
export const competitorSignalProducer: BusinessSignalProducer<CompetitorIntelligenceInput> = {
  ...
  produce({ business, observedAt = new Date().toISOString() }, data) {
    ...
    if (data.pricingGaps?.length) {
      const signal = createSignal({ id: id("competitor", business.business.id, `${observedAt}:pricing-gaps`), businessId: business.business.id, type: "opportunity", source: "competitor-intelligence", metric: "competitor.pricing_gaps", value: data.pricingGaps.length, direction: "negative", confidence: 0.7, context: { gaps: data.pricingGaps }, observedAt });
      signals.push(signal);
      evidence.push(createEvidence({ id: `${signal.id}:evidence`, businessId: business.business.id, type: "observation", source: "competitor-intelligence", observation: `${data.pricingGaps.length} competitor pricing gaps observed.`, data: { gaps: data.pricingGaps, competitors: data.competitors ?? [] }, supportingSignalIds: [signal.id], confidence: 0.7, observedAt }));
    }
    if (data.positioningGaps?.length) {
      const signal = createSignal({ id: id("competitor", business.business.id, `${observedAt}:positioning-gaps`), businessId: business.business.id, type: "opportunity", source: "competitor-intelligence", metric: "competitor.positioning_gaps", value: data.positioningGaps.length, direction: "negative", confidence: 0.65, context: { gaps: data.positioningGaps }, observedAt });
      signals.push(signal);

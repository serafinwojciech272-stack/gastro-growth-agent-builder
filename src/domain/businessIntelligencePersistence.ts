...
function assertBusinessScope(rowBusinessId: string, businessId: string): void {
  if (rowBusinessId !== businessId) throw new Error("Business scope mismatch.");
}
...
export function toBusinessDiagnosisRow(value: Diagnosis): BusinessDiagnosisRow {
  return { id: value.id, business_id: value.businessId, title: value.title, problem: value.problem, symptoms: value.symptoms, root_causes: value.rootCauses, impact: value.impact, confidence: normalizeConfidence(value.confidence), signal_ids: value.signalIds, evidence_ids: value.evidenceIds, alternatives: value.alternatives ?? [], created_at: value.createdAt, updated_at: value.createdAt };
}

export function toBusinessOpportunityRow(value: Opportunity): BusinessOpportunityRow {
  const now = new Date().toISOString();
  return { id: value.id, business_id: value.businessId, title: value.title, description: value.description, source_diagnosis_id: value.sourceDiagnosisId ?? null, impact: value.impact, urgency: value.urgency, confidence: normalizeConfidence(value.confidence), effort: value.effort, cost: value.cost, risk: value.risk, roi: value.roi ?? null, strategic_value: value.strategicValue, time_to_result_days: value.timeToResultDays ?? null, dependencies: value.dependencies, expected_outcome: value.expectedOutcome ?? null, related_kpis: value.relatedKpis, created_at: now, updated_at: now };
}

export function toBusinessRecommendationRow(value: Recommendation): BusinessRecommendationRow {
  const now = new Date().toISOString();
  return { id: value.id, business_id: value.businessId, opportunity_id: value.opportunityId, title: value.title, rationale: value.rationale, actions: value.actions, expected_outcome: value.expectedOutcome, confidence: normalizeConfidence(value.confidence), evidence_ids: value.evidenceIds, policy_version: value.policyVersion, created_at: now, updated_at: now };
}

export function toBusinessPriorityScoreRow(value: PriorityScore, businessId: string): BusinessPriorityScoreRow {
  assertBusinessScope(businessId, businessId);
  return { id: newUuid(), business_id: businessId, opportunity_id: value.opportunityId, score: value.score, rank: value.rank ?? null, policy_version: value.policyVersion, factors: value.factors, explanation: value.explanation, created_at: new Date().toISOString() };
}
...
export async function persistBusinessIntelligenceArtifacts(client: SupabaseClient, artifacts: BusinessIntelligenceArtifacts): Promise<void> {
  for (const diagnosis of artifacts.diagnoses) {
    const row = toBusinessDiagnosisRow(diagnosis);
    assertBusinessScope(row.business_id, diagnosis.businessId);
    const { error } = await client.from("business_diagnoses").upsert(row, { onConflict: "id" });
    if (error) throw new Error(`Diagnosis persistence failed: ${error.message}`);
  }
  for (const opportunity of artifacts.opportunities) {
    const row = toBusinessOpportunityRow(opportunity);
    assertBusinessScope(row.business_id, opportunity.businessId);
    const { error } = await client.from("business_opportunities").upsert(row, { onConflict: "id" });
  ...
  for (const recommendation of artifacts.recommendations) {
    const row = toBusinessRecommendationRow(recommendation);
    assertBusinessScope(row.business_id, recommendation.businessId);
    const { error } = await client.from("business_recommendations").upsert(row, { onConflict: "id" });
    if (error) throw new Error(`Recommendation persistence failed: ${error.message}`);
  }
  for (const priority of artifacts.priorities) {
    const businessId = artifacts.opportunities.find((opportunity) => opportunity.id === priority.opportunityId)?.businessId;
    if (!businessId) throw new Error(`Priority ${priority.opportunityId} references an unknown opportunity.`);
    const row = toBusinessPriorityScoreRow(priority, businessId);
    const { error } = await client.from("business_priority_scores").upsert(row, { onConflict: "opportunity_id,policy_version" });
    if (error) throw new Error(`Priority persistence failed: ${error.message}`);
  }
}

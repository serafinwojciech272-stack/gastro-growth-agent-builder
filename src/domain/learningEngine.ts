...
export function learnFromOutcomes(
  ...
  const byKpi = new Map<string, { deltas: number[]; evidence: string[] }>();
  ...
  for (const outcome of outcomes) {
    for (const [kpi, metric] of Object.entries(outcome.metrics)) {
      ...
      const bucket = byKpi.get(kpi) ?? { deltas: [], evidence: [] };
      bucket.deltas.push(delta);
      bucket.evidence.push(...(outcome.evidence ?? []));
      byKpi.set(kpi, bucket);
    }
  }

  return context.kpis.flatMap((kpi) => {
    const bucket = byKpi.get(kpi.key);
    if (!bucket || bucket.deltas.length === 0) {
      return [{ signal: { kpi: kpi.key, direction: "neutral", confidence: 0, sampleSize: 0, evidence: [] }, recommendation: "collect_more_data" } satisfies LearningRecommendation];
    }
    ...
    const confidence = Math.min(1, bucket.deltas.length / 5);

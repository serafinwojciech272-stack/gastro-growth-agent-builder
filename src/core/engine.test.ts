...
test("Core priority scoring is deterministic and records its policy version", () => {
  const opportunity: Opportunity = {
    id: "opportunity-1", businessId: "business-1", title: "Improve conversion", description: "Test",
    impact: 80, urgency: 70, confidence: 0.9, effort: 20, cost: 10, risk: 10, roi: 75, strategicValue: 60,
    timeToResultDays: 14, dependencies: [], relatedKpis: ["conversion_rate"],
  };
...
test("Mission adapter maps into Core intent without introducing a second state machine", () => {
  const mission: GrowthMission = {
    id: "mission-1", businessId: "business-1", vertical: "restaurant", objective: "Improve conversion",
    ...
    measurementKpis: [{ key: "conversion_rate", label: "Conversion rate", unit: "percentage" }], status: "awaiting_approval",
  };
...
test("Core outcome adapter preserves insufficient-data semantics and derives reusable learning", () => {
  const mission: GrowthMission = {
    id: "mission-2", businessId: "business-1", vertical: "generic_business", objective: "Test outcome", actions: [],
    measurementKpis: [{ key: "revenue", label: "Revenue", unit: "currency" }], status: "measuring",
  };
  const outcome = recordCoreOutcome({ mission, confidence: 0.9, metrics: { revenue: { before: 100, after: 120, delta: 20 } }, evidence: ["evidence-1"] });
  const learning = toCoreLearning({ ...outcome, learning: undefined });
...
test("Provenance rejects unverifiable observed evidence and accepts explicit lineage", () => {
  const event = {
    id: "run-1:evidence", runId: "run-1", stage: "evidence" as const, eventType: "evidence.built",
    timestamp: "2026-01-02T00:00:01.000Z", payload: { count: 1 },
  };
  const observed = attachProvenance(event, [{ source: "analytics", sourceId: "metric-1", observedAt: "2026-01-02T00:00:00.000Z", truthStatus: "observed", lineageIds: ["signal-1"] }]);
  assert.deepEqual(observed.provenance?.[0], createProvenance({ source: "analytics", sourceId: "metric-1", observedAt: "2026-01-02T00:00:00.000Z", truthStatus: "observed", lineageIds: ["signal-1"] }));
  assert.deepEqual(validateTrace([observed]), []);
  assert.ok(validateTrace([{ ...event, provenance: [{ source: "analytics", truthStatus: "observed" }] }]).includes("observed_without_observed_at:run-1:evidence"));
});

test("Trace replay preserves deterministic chronological ordering and detects stage regression", () => {
  const trace = [

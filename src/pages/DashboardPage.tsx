  ...
  async function loadData(userId: string) {
    ...
    try { const sb = requireSupabase();
      ...
      const [{ data: ms, error: me }, { data: mt, error: mte }, { data: lm, error: le }] = await Promise.all([
        ...
        sb.from("growth_measurements").select("id,mission_id,kpi,before_value,after_value,delta,confidence,measured_at").eq("restaurant_id", r.id).order("measured_at", { ascending: false }).limit(12),
        sb.from("growth_learning_memory").select("id,kpi,recommendation,confidence,sample_size,created_at").eq("restaurant_id", r.id).order("created_at", { ascending: false }).limit(8),
      ]);

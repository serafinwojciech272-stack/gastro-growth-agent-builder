...
export async function analyzeMenu(menuText: string): Promise<string> {
  ...
  const { data, error } = await supabase.functions.invoke("gga-menu-analyzer", { body: { menuText: input } });
  if (error) {
  ...
  if (typeof analysis !== "string" || !analysis.trim()) throw new Error("Die KI hat keine verwertbare Analyse zurückgegeben.");

...
function decodeHtml(bytes: Uint8Array): string { return new TextDecoder("utf-8", { fatal: false }).decode(bytes); }
...
function countMatches(html: string, expression: RegExp): number { return html.match(expression)?.length ?? 0; }

function scoreWebsite(input: { url: URL; status: number; responseTimeMs: number; html: string; contentType?: string }): WebsiteAuditSnapshot & { facts: Record<string, boolean | number | string | undefined> } {
  ...
  return {
    url: url.toString(), performanceScore, seoScore, mobileScore, accessibilityScore, trustScore, issues,
    facts: { status, responseTimeMs, https, title: title ?? "", hasMetaDescription: description, hasViewport: viewport, hasCanonical: canonical, hasRobotsMeta: robots, hasHtmlLang: lang, h1Count, imageCount: images, imagesWithAlt, contentLength: textLength },
  };
}
...
export async function collectWebsiteAudit(urlInput: string, options: WebsiteAuditCollectorOptions = {}): Promise<WebsiteAuditResult> {
  ...
  for (let redirect = 0; redirect <= maxRedirects; redirect += 1) {
    ...
    try {
      response = await fetchImpl(url, { method: "GET", redirect: "manual", signal: controller.signal, headers: { "user-agent": "GGA-WebsiteAudit/1.0 (+business-intelligence)" } });
  ...
  return { ...scored, status: response.status, contentType, responseTimeMs, finalUrl: url.toString(), observedAt: new Date().toISOString() };
}

...
export default function AIMenuScanner() {
  ...
  const handleAnalyze = (e: React.FormEvent) => {
    ...
    // Simulated deterministic AI analysis
    setTimeout(() => {
      const missingItems = menu.toLowerCase().includes("dessert") ? "" : "Fehlende Desserts erkannt. ";
      const pricingIssue = menu.match(/€\d+/) ? "Potenzielle Preisoptimierung bei Hauptgerängen erkannt. " : "";
      const crossSelling = "Cross-Selling Potential: Getränke-Pakete vorschlagen. ";
      
      setAnalysis(`Analyse abgeschlossen: ${missingItems}${pricingIssue}${crossSelling}Erwartete Umsatzsteigerung: 8-12%.`);
      setLoading(false);
    }, 1500);
  };
